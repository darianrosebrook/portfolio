...
test.describe('Foundation Pages Navigation', () => {
  test.describe('Page Loading', () => {
    for (const pageData of foundationPages) {
      test(`should load ${pageData.slug} page without errors`, async ({
        page,
      }) => {
        const errors: string[] = [];
        page.on('console', (msg) => {
  ...
  test.describe('Prerequisites Navigation', () => {
    for (const pageData of foundationPages) {
      if (pageData.prerequisites.length > 0) {
        test(`should navigate to prerequisites from ${pageData.slug}`, async ({
          page,
        }) => {
          await page.goto(`/blueprints/foundations/${pageData.slug}`);

          for (const prereq of pageData.prerequisites) {
            const prereqLink = page.locator(
              `nav[aria-label="Prerequisites"] a[href*="${prereq}"]`
            );
            await expect(prereqLink).toBeVisible({ timeout: 5000 });

            await prereqLink.click();
            await expect(page).toHaveURL(
              new RegExp(`/blueprints/foundations/${prereq}`)
            );

            // Navigate back
            await page.goBack();
            await expect(page).toHaveURL(
              new RegExp(`/blueprints/foundations/${pageData.slug}`)
            );
          }
        });
      }
  ...
  test.describe('Next Units Navigation', () => {
    for (const pageData of foundationPages) {
      if (pageData.nextUnits.length > 0) {
        test(`should navigate to next units from ${pageData.slug}`, async ({
          page,
        }) => {
          await page.goto(`/blueprints/foundations/${pageData.slug}`);

          for (const nextUnit of pageData.nextUnits) {
            const nextUnitLink = page.locator(
              `nav[aria-label="Next steps"] a[href*="${nextUnit}"]`
            );
            await expect(nextUnitLink).toBeVisible({ timeout: 5000 });

            await nextUnitLink.click();
            await expect(page).toHaveURL(
              new RegExp(`/blueprints/foundations/${nextUnit}`)
            );
  ...
  test.describe('Cross-Reference Links', () => {
    ...
    test('should navigate to component references', async ({ page }) => {
      ...
      if (linkCount > 0) {
        ...
        await expect(page).toHaveURL(
          new RegExp('/blueprints/component-standards/')
        );
      }
    });
  });
  ...
  test.describe('Glossary Integration', () => {
    test('should display glossary terms in cross-references', async ({
      page,
    }) => {
      await page.goto('/blueprints/foundations/philosophy');
  ...
  test.describe('JSON-LD Structured Data', () => {
    for (const pageData of foundationPages) {
      test(`should have valid JSON-LD schemas on ${pageData.slug}`, async ({
        page,
      }) => {
        await page.goto(`/blueprints/foundations/${pageData.slug}`);
      ...
      test(`should have BreadcrumbList schema on ${pageData.slug}`, async ({
        page,
      }) => {
        await page.goto(`/blueprints/foundations/${pageData.slug}`);
  ...
  test.describe('SEO Metadata', () => {
    for (const pageData of foundationPages) {
      test(`should have correct SEO metadata on ${pageData.slug}`, async ({
        page,
      }) => {
        await page.goto(`/blueprints/foundations/${pageData.slug}`);
        ...
        if ((await canonical.count()) > 0) {
          ...
          expect(canonicalUrl).toContain(
            `/blueprints/foundations/${pageData.slug}`
          );
        }
  ...
  test.describe('Metrics Tracking', () => {
    ...
    test('should track cross-reference clicks', async ({ page, context }) => {
      ...
      const crossRefLink = page
        .locator('.conceptGrid a, .componentReferences a')
        .first();
