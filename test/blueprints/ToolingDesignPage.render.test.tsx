...
import ToolingDesignPage, {
  metadata,
} from '@/app/blueprints/foundations/tooling/design/page';

/**
 * Render-level contract for the Design Tooling page: template sections
 * present, real tooling citations (variable mirroring, the authority
 * rule, shared contrast math, structural drift detection), distinct
...
describe('Design Tooling page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe('Design Tooling | Darian Rosebrook');
  ...
  it('renders the education template header and no placeholder', () => {
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: 'Design Tooling' })
    ).toBeTruthy();
  ...
  it('cites the real tooling mechanisms', () => {
    renderPage();
