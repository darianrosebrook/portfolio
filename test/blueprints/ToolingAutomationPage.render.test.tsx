...
import ToolingAutomationPage, {
  metadata,
} from '@/app/blueprints/foundations/tooling/automation/page';

/**
 * Render-level contract for the Automation & CI/CD page: template
 * sections present, real automation citations (lint-staged config,
 * the pre-push gate shape, the build-first deploy, the honest CI gap
 * and its wiring), distinct interplay tabs, no placeholder.
 */

function renderPage() {
  return render(
    <ReducedMotionProvider>
      <ToolingAutomationPage />
    </ReducedMotionProvider>
  );
}

describe('Automation & CI/CD page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe('Automation & CI/CD | Darian Rosebrook');
  ...
  it('renders the education template header and no placeholder', () => {
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: 'Automation & CI/CD' })
    ).toBeTruthy();
  ...
  it('cites the real automation layers honestly', () => {
    renderPage();
