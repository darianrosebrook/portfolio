...
import FoundationsAccessibilityStandardsPage, {
  metadata,
} from '@/app/blueprints/foundations/accessibility/standards/page';

/**
 * Render-level contract for the Accessibility Standards & Principles
 * page: template sections present, real citations (WCAG_LEVELS
 * constants, the criteria-to-file table, POUR, ARIA's first rule),
...
function renderPage() {
  return render(
    <ReducedMotionProvider>
      <FoundationsAccessibilityStandardsPage />
    </ReducedMotionProvider>
  );
}

describe('Accessibility Standards & Principles page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe(
      'Accessibility Standards & Principles | Darian Rosebrook'
    );
  ...
  it('renders the education template header and no placeholder', () => {
    ...
    expect(
      screen.getByRole('heading', {
        level: 1,
        name: 'Accessibility Standards & Principles',
      })
    ).toBeTruthy();
  ...
  it('cites the real thresholds and criteria', () => {
    renderPage();
