...
import NavigationPage, {
  metadata,
} from '@/app/blueprints/ux-patterns/navigation/page';

/**
 * Render-level contract for the Navigation Patterns page: template
 * sections present, real citations (the tab triad with wiring,
 * aria-current, breadcrumb nav labeling, the skip link), distinct
...
describe('Navigation Patterns page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe('Navigation Patterns | Darian Rosebrook');
    expect(metadata.openGraph?.url).toBe(
      'https://darianrosebrook.com/blueprints/ux-patterns/navigation'
    );
  });
  ...
  it('renders the education template header and no placeholder', () => {
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: 'Navigation Patterns' })
    ).toBeTruthy();
  ...
  it('cites the real navigation wiring', () => {
    renderPage();
