...
import MetaSystemVsStylePage, {
  metadata,
} from '@/app/blueprints/foundations/meta/system-vs-style/page';

/**
 * Render-level contract for the meta System vs Style page: template
 * sections present, the two-ledger doctrine with its rebrand test and
 * provenance rule, distinct interplay tabs, no placeholder.
 */

function renderPage() {
  return render(
    <ReducedMotionProvider>
      <MetaSystemVsStylePage />
    </ReducedMotionProvider>
  );
}

describe('System vs Style (meta) page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe('System vs Style | Darian Rosebrook');
  ...
  it('renders the education template header and no placeholder', () => {
    renderPage();
    expect(
      screen.getByRole('heading', { level: 1, name: 'System vs Style' })
    ).toBeTruthy();
  ...
  it('teaches the doctrine with its key tests and rules', () => {
    renderPage();
    const text = document.body.textContent ?? '';
    for (const s of [
      'rebrand test',
      'derived answers are system',
      'chosen answers are style',
      'three-field',
