...
import RadiusPage, { metadata } from '@/app/blueprints/foundations/radius/page';

/**
 * Render-level contract for the Radius & Shape Foundations page:
 * template sections present, real shape citations (scale values,
 * semantic role mapping, the pill contract, focus-width pairing),
...
describe('Radius & Shape Foundations page', () => {
  ...
  it('cites real shape tokens and values', () => {
    renderPage();
  ...
  it('renders checklist, assessments, and cross-references', () => {
    ...
    expect(
      links.some((href) => href.includes('/blueprints/foundations/borders'))
    ).toBe(true);
    expect(
