...
import MetaComponentMappingPage, {
  metadata,
} from '@/app/blueprints/foundations/meta/component-mapping/page';

/**
 * Render-level contract for the meta Component Token Mapping page:
 * template sections present, real contract citations (slots with
 * selectors, the four-field binding sentence, scoped emission,
 * generated anatomy docs), distinct interplay tabs, no placeholder.
 */

function renderPage() {
  return render(
    <ReducedMotionProvider>
      <MetaComponentMappingPage />
    </ReducedMotionProvider>
  );
}

describe('Component Token Mapping (meta) page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe('Component Token Mapping | Darian Rosebrook');
  ...
  it('cites the real contract machinery', () => {
    renderPage();
