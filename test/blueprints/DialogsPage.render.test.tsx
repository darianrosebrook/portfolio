...
import DialogsPage, {
  metadata,
} from '@/app/blueprints/ux-patterns/dialogs/page';

/**
 * Render-level contract for the Dialogs & Overlay Patterns page:
 * template sections present, real citations (Dialog contract role and
 * keyboard, focus invariants, modal composites, tooltip delays,
 * depth agreement), distinct interplay tabs, no placeholder.
 */
...
describe('Dialogs & Overlay Patterns page', () => {
  it('exports SEO metadata derived from the page content block', () => {
    expect(metadata.title).toBe(
      'Dialogs & Overlay Patterns | Darian Rosebrook'
    );
    expect(metadata.openGraph?.url).toBe(
      'https://darianrosebrook.com/blueprints/ux-patterns/dialogs'
    );
  });
  ...
  it('cites the real overlay machinery', () => {
    renderPage();
  ...
  it('renders checklist, assessments, and cross-references', () => {
    ...
    expect(
      links.some((href) => href.includes('/blueprints/foundations/elevation'))
    ).toBe(true);
    expect(
