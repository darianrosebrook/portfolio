...
const Tag: React.FC<TagProps> = ({
  ...
  const safeShape = safeTokenValue(shape, 'rounded', (v) =>
    ['rounded', 'pill', 'square'].includes(v as string)
  ) as string;
  ...
  return (
    ...
    >
      {icon && iconPosition === 'start' && (
        <TagIcon size={safeSize as any} position="start">
          {icon}
        </TagIcon>
      )}
      ...
      {icon && iconPosition === 'end' && (
        <TagIcon size={safeSize as any} position="end">
          {icon}
        </TagIcon>
      )}
