...
for (const [, varName, value] of cssContent.matchAll(
  /--([a-z][a-z0-9-]+):\s*([^;v][^;]*);/g
)) {
  const v = value.trim();
...
for (const [, lv, rv] of scss.matchAll(
...
for (const [part, tokenValue] of Object.entries(contract.tokens)) {
  ...
  for (const tokenName of tokenValue) {
    ...
    if (!semanticVar || !fallback) {
      ...
      console.log(
        `    localVar: ${localVar}, semanticVar: ${semanticVar ?? 'NOT_IN_BRIDGE'}, fallback: ${fallback ? fallback.slice(0, 30) : 'EMPTY'}`
      );
      canConvert = false;
    }
  }
