...
function checkTier1(component) {
  ...
  if (component.paths?.component) {
    ...
    if (!pathExists) {
      issues.push(`Component path does not exist: ${component.paths.component}`);
    }
  } else {
  ...
  if (pathExists && component.paths?.component) {
    ...
    if (fs.existsSync(indexPath)) {
      ...
      const hasDefaultExport = /export\s+(default\s+)?.*from/.test(indexContent);
...
function checkTier2(component) {
  ...
  const generatedPath = path.join(componentPath, `${componentName}.tokens.generated.scss`);
  ...
  const passed = Object.values(checks).filter((v) => v !== undefined).every((v) => v === true);
...
function checkTier3(component) {
  ...
  if (fs.existsSync(mainFile)) {
    ...
    checks.hasReactImport = /import.*React/.test(content) || /from\s+['"]react['"]/.test(content);
    ...
    const hasComponentExport = 
      ...
      /const\s+\w+\s*[:=]\s*(React\.FC|React\.FunctionComponent)/.test(content) ||
    ...
    if (!checks.hasReactImport) issues.push('Main component file missing React import');
    if (!checks.hasComponentExport) issues.push('Main component file missing component export');
  } else {
  ...
  if (fs.existsSync(examplesPath)) {
    ...
    const hasCustomGenerator = new RegExp(`case\\s+['"]${component.component}['"]`).test(examplesContent);
    checks.hasCustomExampleGenerator = hasCustomGenerator;
    if (!hasCustomGenerator) {
      issues.push('Using generic example generator (custom generator recommended)');
    }
  } else {
  ...
  if (fs.existsSync(examplesPath)) {
    const examplesContent = fs.readFileSync(examplesPath, 'utf8');
    const hasCustomVariantGrid = new RegExp(`case\\s+['"]${component.component}['"]`).test(examplesContent);
  ...
  if (fs.existsSync(examplesPath)) {
    const examplesContent = fs.readFileSync(examplesPath, 'utf8');
    const hasCustomControls = new RegExp(`case\\s+['"]${component.component}['"]`).test(examplesContent);
  ...
  const passed = checks.hasMainFile && checks.hasReactImport && checks.hasComponentExport;
...
function checkTier4(component) {
    ...
    try {
      ...
      if (contract.a11y) {
        ...
        if (!checks.hasA11yKeyboard && component.status === 'Built') {
          issues.push('Contract missing a11y.keyboard (recommended for interactive components)');
        }
  ...
  // A12: Component Tests
  const testPath = path.join(componentPath, 'tests', `${componentName}.test.tsx`);
  if (fs.existsSync(testPath)) {
    ...
    if (!checks.testsHaveDescribe) issues.push('Test file missing describe block');
  ...
  if (fs.existsSync(mainFile)) {
    ...
    checks.hasComponentType = /React\.FC|React\.FunctionComponent|function\s+\w+/.test(content);
    
    if (!checks.hasPropsType) issues.push('Component missing Props type export');
    if (!checks.hasComponentType) issues.push('Component missing proper type definition');
  } else {
...
function validateComponent(component) {
  ...
  const { overall, score, maxScore } = calculateOverall(tier1, tier2, tier3, tier4);
...
function printReport(report) {
  ...
  log(statusColors[report.overall], `   Status: ${report.status} | Visibility: ${report.overall.toUpperCase()} | Score: ${report.score}/${report.maxScore}`);
  ...
  if (report.tier2.issues.length > 0) {
    log(colors.yellow, `   ⚠️  Tier 2 (Documentation) - ${report.tier2.passed ? 'PASSED' : 'ISSUES'}`);
    report.tier2.issues.forEach((issue) => log(colors.yellow, `      • ${issue}`));
  } else {
    log(colors.green, '   ✅ Tier 2 (Documentation) - PASSED');
  }

  if (report.tier3.issues.length > 0) {
    log(colors.yellow, `   ⚠️  Tier 3 (Interactive Examples) - ${report.tier3.passed ? 'PASSED' : 'ISSUES'}`);
    report.tier3.issues.forEach((issue) => log(colors.yellow, `      • ${issue}`));
  } else {
    log(colors.green, '   ✅ Tier 3 (Interactive Examples) - PASSED');
  }

  if (report.tier4.issues.length > 0) {
    log(colors.yellow, `   ⚠️  Tier 4 (Best Practices) - ${report.tier4.passed ? 'PASSED' : 'ISSUES'}`);
    report.tier4.issues.forEach((issue) => log(colors.yellow, `      • ${issue}`));
  } else {
...
function main() {
  ...
  if (!fs.existsSync(TRANSFORMED_JSON)) {
    log(colors.red, `❌ Cannot find components-transformed.json at ${TRANSFORMED_JSON}`);
    process.exit(1);
  }
