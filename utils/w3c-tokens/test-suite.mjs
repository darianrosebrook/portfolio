...
function test(name, tokens, shouldPass = true, expectedErrors = []) {
  ...
  if (isValid) {
    ...
    console.log(`❌ ${name}`);
    if (errors.length > 0) {
      console.log(`   Expected: ${shouldPass ? 'valid' : 'invalid'}, Got: ${shouldPass ? 'invalid' : 'valid'}`);
...
test('Valid typography composite token', {
  typography: {
    heading: {
      $type: 'typography',
      $value: {
...
test('Valid group with type inheritance', {
  spacing: {
    $type: 'dimension',
    small: {
      $value: {
...
test('Valid fontWeight token (numeric)', {
  typography: {
    bold: {
      $type: 'fontWeight',
      $value: 700
...
test('Valid fontWeight token (named)', {
  typography: {
    bold: {
      $type: 'fontWeight',
      $value: 'bold'
...
test('Complex nested token structure', {
  color: {
    palette: {
      primary: {
        base: {
