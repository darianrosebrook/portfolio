export const SVGComponentIllo = (
  <svg style={{ display: 'none' }} id="DSSPRITE">
    <symbol viewBox="0 0 216 97" id="pin">
      <rect
        width="100"
        height="20"
        rx="4"
        fill="var(--semantic-color-background-inverse)"
      />
      <rect
        y="85"
        width="100"
        height="12"
        rx="4"
        fill="var(--semantic-color-foreground-secondary)"
      />
      <rect
        x="0.5"
        y="28.5"
        width="47"
        height="47"
        rx="7.5"
        fill="var(--semantic-color-background-secondary)"
        stroke="var(--semantic-color-foreground-tertiary)"
      />
      <rect
        x="56.5"
        y="28.5"
        width="47"
        height="47"
        rx="7.5"
        fill="var(--semantic-color-background-secondary)"
        stroke="var(--semantic-color-border-bold)"
      />
      <rect
        x="112.5"
        y="28.5"
        width="47"
        height="47"
        rx="7.5"
        fill="var(--semantic-color-background-secondary)"
        stroke="var(--semantic-color-border-bold)"
      />
      <rect
        x="168.5"
        y="28.5"
        width="47"
        height="47"
        rx="7.5"
        fill="var(--semantic-color-background-secondary)"
        stroke="var(--semantic-color-border-bold)"
      />
      <rect
        x="15"
        y="66"
        width="18"
        height="2"
        rx="1"
        fill="var(--semantic-color-foreground-placeholder)"
      />
      <rect
        x="71"
        y="66"
        width="18"
        height="2"
        rx="1"
        fill="var(--semantic-color-foreground-placeholder)"
      />
      <rect
        x="127"
        y="66"
        width="18"
        height="2"
        rx="1"
        fill="var(--semantic-color-foreground-placeholder)"
      />
      <rect
        x="183"
        y="66"
        width="18"
        height="2"
        rx="1"
        fill="var(--semantic-color-foreground-placeholder)"
      />
    </symbol>
    <symbol viewBox="0 0 327 56" id="input">
      <rect
        height="55"
        width="326"
        fill="var(--semantic-color-background-primary)"
        rx="3.5"
        x="0.5"
        y="0.5"
      />
      <rect
        height="55"
        width="326"
        rx="3.5"
        fill="none"
        stroke="var(--semantic-color-foreground-tertiary)"
        x="0.5"
        y="0.5"
      />
      <rect
        height="24"
        width="24"
        fill="var(--semantic-color-foreground-tertiary)"
        rx="4"
        x="16"
        y="16"
      />
      <rect
        height="16"
        width="86"
        fill="var(--semantic-color-background-inverse)"
        rx="4"
        x="48"
        y="20"
      />
      <rect
        height="16"
        width="38"
        fill="var(--semantic-color-background-inverse)"
        rx="4"
        x="136"
        y="20"
      />
      <rect
        height="40"
        width="1"
        fill="var(--semantic-color-foreground-placeholder)"
        transform="translate(278 8)"
      />
      <rect
        height="24"
        width="24"
        fill="var(--semantic-color-foreground-tertiary)"
        rx="4"
        x="246"
        y="16"
      />
      <rect
        height="24"
        width="24"
        fill="var(--semantic-color-foreground-tertiary)"
        rx="4"
        x="287"
        y="16"
      />
    </symbol>
    <symbol viewBox="0 0 300 241" id="modal">
      <rect
        height="238.696"
        width="299.348"
        fill="var(--semantic-color-background-primary)"
        rx="4.8913"
        x="0.326087"
        y="1.26066"
        stroke="#CCCCCC"
        strokeWidth="0.652174"
      />
      <rect
        height="23.4783"
        width="94.5652"
        fill="var(--semantic-color-background-inverse)"
        rx="11.7391"
        x="189.783"
        y="201.152"
      />
      <rect
        height="52.1739"
        width="268.696"
        fill="var(--semantic-color-background-tertiary)"
        rx="5.21739"
        x="15.6523"
        y="117.674"
      />
      <rect
        height="45.6522"
        width="298.043"
        fill="var(--semantic-color-background-primary)"
        transform="translate(0.652344 56.3691)"
      />
      <rect
        height="0.652174"
        width="298.043"
        fill="var(--semantic-color-border-subtle)"
        x="0.652344"
        y="101.37"
      />
      <rect
        height="1.30435"
        width="34.5652"
        fill="var(--semantic-color-background-inverse)"
        x="16.3047"
        y="100.718"
      />
      <rect
        height="15.6522"
        width="34.5652"
        fill="var(--semantic-color-background-inverse)"
        rx="2.6087"
        x="15.6528"
        y="73.979"
      />
      <rect
        height="15.6522"
        width="43.6957"
        fill="var(--semantic-color-background-inverse)"
        rx="2.6087"
        x="74.3481"
        y="73.979"
      />
      <rect
        height="10.4348"
        width="50.2174"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="28.6958"
        y="130.717"
      />
      <rect
        height="10.4348"
        width="50.2174"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="105.652"
        y="146.369"
      />
      <rect
        height="10.4348"
        width="72.3913"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="83.478"
        y="130.717"
      />
      <rect
        height="10.4348"
        width="72.3913"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="28.0435"
        y="146.369"
      />
      <rect
        height="10.4348"
        width="72.3913"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="161.087"
        y="130.717"
      />
      <rect
        height="10.4348"
        width="32.6087"
        fill="var(--semantic-color-background-primary)"
        rx="2.6087"
        x="200.87"
        y="207.674"
      />
      <rect
        height="10.4348"
        width="39.1304"
        fill="var(--semantic-color-background-primary)"
        rx="2.6087"
        x="237.391"
        y="207.674"
      />
      <rect
        height="15.6522"
        width="172.174"
        fill="var(--semantic-color-background-inverse)"
        rx="2.6087"
        x="15.6523"
        y="20.5"
      />
      <rect
        height="10.4348"
        width="10.4348"
        fill="var(--semantic-color-foreground-primary)"
        rx="2.6087"
        x="266.087"
        y="23.1084"
      />
      <line
        stroke="var(--semantic-color-border-subtle)"
        strokeWidth="0.652174"
        x1="-2.85074e-08"
        x2="298.696"
        y1="55.3912"
        y2="55.3912"
      />
      <line
        stroke="var(--semantic-color-border-subtle)"
        strokeWidth="0.652174"
        x1="-2.85074e-08"
        x2="300"
        y1="185.174"
        y2="185.174"
      />
      <circle
        cx="271.305"
        cy="28.326"
        fill="#F5F5F5"
        r="11.413"
        stroke="var(--semantic-color-border-bold)"
        strokeWidth="0.652174"
      />
    </symbol>
    <symbol viewBox="0 0 289 48" id="toggleButtonGroup">
      <rect
        height="47"
        width="288"
        rx="23.5"
        stroke="var(--semantic-color-border-bold)"
        fill="none"
        x="0.5"
        y="0.5"
      />
      <rect
        height="40"
        width="140"
        fill="var(--semantic-color-background-inverse)"
        rx="20"
        x="4"
        y="4"
      />
      <rect
        height="16"
        width="83"
        fill="var(--semantic-color-foreground-placeholder)"
        rx="4"
        x="168"
        y="16"
      />
    </symbol>
    <symbol viewBox="0 0 334 194" id="timeline">
      <path
        d="M24 3C26.2091 3 28 4.79086 28 7C28 9.20914 26.2091 11 24 11C21.7909 11 20 9.20914 20 7C20 4.79086 21.7909 3 24 3Z"
        fill="var(--semantic-color-foreground-placeholder)"
        fillRule="evenodd"
      />
      <path
        d="M24 79C26.2091 79 28 80.7909 28 83C28 85.2091 26.2091 87 24 87C21.7909 87 20 85.2091 20 83C20 80.7909 21.7909 79 24 79Z"
        fill="var(--semantic-color-foreground-placeholder)"
        fillRule="evenodd"
      />
      <rect
        height="16"
        width="246"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="36"
        y="76"
      />
      <rect
        height="16"
        width="113"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="36"
        y="94"
      />
      <rect
        height="16"
        width="246"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="36"
      />
      <rect
        height="16"
        width="113"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="36"
        y="18"
      />
      <rect
        height="0.999998"
        width="48"
        fill="var(--semantic-color-foreground-placeholder)"
        transform="translate(24 21) rotate(90)"
      />
      <rect
        height="0.999998"
        width="48"
        fill="var(--semantic-color-foreground-placeholder)"
        transform="translate(24 97) rotate(90)"
      />
      <rect
        height="64"
        width="334"
        fill="var(--semantic-color-background-secondary)"
        rx="32"
        y="130"
      />
      <rect
        height="40"
        width="40"
        fill="var(--semantic-color-foreground-primary)"
        rx="20"
        x="12"
        y="142"
      />
      <rect
        height="16"
        width="16"
        fill="var(--semantic-color-foreground-inverse)"
        rx="4"
        x="24"
        y="154"
      />
      <rect
        height="16"
        width="246"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="64"
        y="145"
      />
      <rect
        height="16"
        width="113"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="64"
        y="163"
      />
    </symbol>
    <symbol viewBox="0 0 103 85" id="tooltip">
      <circle
        cx="52"
        cy="65"
        r="19.4444"
        fill="none"
        stroke="var(--semantic-color-border-bold)"
        strokeWidth="1.11111"
      />
      <rect
        height="17.9995"
        width="17.9995"
        fill="var(--semantic-color-border-bold)"
        rx="4.49986"
        x="43"
        y="56"
      />
      <rect
        height="32"
        width="103"
        fill="var(--semantic-color-background-secondary)"
        rx="8"
      />
      <path
        d="M40 31.3135L51.3137 19.9998L62.6274 31.3135L52.7279 41.213C51.9469 41.994 50.6805 41.994 49.8995 41.213L40 31.3135Z"
        fill="var(--semantic-color-background-secondary)"
      />
    </symbol>
    <symbol fill="none" viewBox="0 0 300 36" id="pagination">
      <circle
        cx="16"
        cy="18"
        fill="var(--semantic-color-background-inverse)"
        r="16"
      />
      <circle
        cx="284"
        cy="18"
        fill="var(--semantic-color-background-inverse)"
        r="16"
      />
      <rect
        height="16"
        width="16"
        fill="var(--semantic-color-foreground-inverse)"
        rx="4"
        x="8"
        y="10"
      />
      <rect
        height="16"
        width="16"
        fill="var(--semantic-color-foreground-inverse)"
        rx="4"
        x="276"
        y="10"
      />
      <rect
        height="16"
        width="12"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="56"
        y="10"
      />
      <rect
        height="16"
        width="12"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="100"
        y="10"
      />
      <rect
        height="16"
        width="12"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="144"
        y="10"
      />
      <rect
        height="16"
        width="12"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="232"
        y="10"
      />

      <path
        d="M72 31C72 31.5523 71.5523 32 71 32H53C52.4477 32 52 31.5523 52 31V31C52 30.4477 52.4477 30 53 30H71C71.5523 30 72 30.4477 72 31V31Z"
        fill="var(--semantic-color-foreground-primary)"
        fillRule="evenodd"
      />
      <path
        d="M197.5 18C197.5 18.8284 198.172 19.5 199 19.5C199.828 19.5 200.5 18.8284 200.5 18C200.5 17.1716 199.828 16.5 199 16.5C198.172 16.5 197.5 17.1716 197.5 18Z"
        fill="var(--semantic-color-foreground-secondary)"
      />
      <path
        d="M187.5 18C187.5 18.8284 188.172 19.5 189 19.5C189.828 19.5 190.5 18.8284 190.5 18C190.5 17.1716 189.828 16.5 189 16.5C188.172 16.5 187.5 17.1716 187.5 18Z"
        fill="var(--semantic-color-foreground-secondary)"
      />
      <path
        d="M194 19.5C193.172 19.5 192.5 18.8284 192.5 18C192.5 17.1716 193.172 16.5 194 16.5C194.828 16.5 195.5 17.1716 195.5 18C195.5 18.8284 194.828 19.5 194 19.5Z"
        fill="var(--semantic-color-foreground-secondary)"
      />
    </symbol>
    <symbol viewBox="0 0 375 205" id="metrics">
      <rect
        height="205"
        width="375"
        stroke="var(--semantic-color-border-bold)"
        rx="4"
        fill="var(--semantic-color-background-primary)"
      />
      <rect
        height="1"
        width="375"
        fill="var(--semantic-color-border-bold)"
        transform="translate(0 137)"
      />
      <rect
        height="16"
        width="57"
        fill="var(--semantic-color-foreground-placeholder)"
        rx="4"
        x="300"
        y="105"
      />
      <rect
        height="47"
        width="47"
        fill="var(--semantic-color-background-secondary)"
        rx="7.5"
        stroke="var(--semantic-color-foreground-secondary)"
        x="233.5"
        y="147.5"
      />
      <rect
        height="26"
        width="16"
        fill="var(--semantic-color-foreground-placeholder)"
        rx="4"
        x="249"
        y="159"
      />
      <rect
        height="16"
        width="32"
        fill="var(--semantic-color-foreground-tertiary)"
        rx="4"
        x="17"
        y="18"
      />
      <rect
        height="16"
        width="82"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="180"
        y="43"
      />
      <rect
        height="16"
        width="82"
        fill="var(--semantic-color-foreground-tertiary)"
        rx="4"
        x="17"
        y="164"
      />
      <rect
        height="16"
        width="79"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="278"
        y="68"
      />
      <rect
        height="16"
        width="23"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="266"
        y="43"
      />
      <rect
        height="16"
        width="16"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="293"
        y="163"
      />
      <rect
        height="16"
        width="27"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="313"
        y="163"
      />
      <rect
        height="16"
        width="64"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="293"
        y="43"
      />
      <rect
        height="16"
        width="64"
        fill="var(--semantic-color-foreground-secondary)"
        rx="4"
        x="210"
        y="68"
      />
      <rect
        height="41"
        width="58"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="17"
        y="51"
      />
      <rect
        height="41"
        width="41"
        fill="var(--semantic-color-foreground-primary)"
        rx="4"
        x="79"
        y="51"
      />
      <path
        d="M206.967 177.966L200 171L206.967 164.033"
        stroke="var(--semantic-color-border-bold)"
        fill="none"
        strokeWidth="1.5"
      />
      <path
        d="M349.915 177.966L356.882 171L349.915 164.033"
        stroke="var(--semantic-color-border-bold)"
        fill="none"
        strokeWidth="1.5"
      />

      <path
        d="M194 80.5V68"
        stroke="var(--semantic-color-foreground-accent)"
        fill="none"
        strokeWidth="1.5"
      />
      <path
        d="M188.5 75.5L194 81L199.5 75.5"
        stroke="var(--semantic-color-foreground-accent)"
        fill="none"
        strokeWidth="1.5"
      />
    </symbol>
  </svg>
);
