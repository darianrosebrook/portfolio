...
/**
 * GET: outgoing relations + incoming backlinks for a case study.
 * Public read (RLS allows anonymous SELECT); the case study itself must be
...
export async function GET(
  ...
  if (error) {
    console.error(
      'Case study relations fetch error:',
  ...
  const [relations, backlinks] = await Promise.all([
    getRelations(supabase, caseStudy.id, 'case-study'),
    getBacklinks(supabase, caseStudy.id, 'case-study'),
  ]);
...
/**
 * PUT: replace the full relation set a case study declares. Author only.
...
export async function PUT(
  ...
  const { data: caseStudy, error: lookupError } = await supabase
    .from('case_studies')
    .select('id')
    .eq('slug', slug)
    .eq('author', user.id)
  ...
  if (lookupError) {
    console.error(
      'Case study relations lookup error:',
  ...
  const relations = await getRelations(supabase, caseStudy.id, 'case-study');
  return new NextResponse(
