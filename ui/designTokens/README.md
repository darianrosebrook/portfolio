...
- TypeScript token paths: `types/designTokens.ts` from `utils/designTokens/generators/types.ts` (reads the composed `designTokens.json`) for code-time autocomplete via `TokenPath` and helpers.
